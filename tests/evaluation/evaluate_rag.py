...
class MockRetriever:
    async def search_with_reranking(self, query, **kwargs):
        ...
        if "capital" in q_lower and "pma" in q_lower:
            return {"results": [{"text": "The minimum paid-up capital for a PT PMA is 10 billion IDR (excluding land and buildings).", "metadata": {"title": "BKPM Reg 4/2021"}}]}
        if "investor kitas" in q_lower:
            return {"results": [{"text": "Investor KITAS holders can act as Directors/Commissioners but cannot perform manual labor or full-time employment roles meant for locals.", "metadata": {"title": "Visa Regulations"}}]}
        if "freehold" in q_lower or "hak milik" in q_lower:
            return {"results": [{"text": "Foreigners cannot own Freehold (Hak Milik) land in Indonesia. They can only hold Right to Use (Hak Pakai) or Leasehold (Hak Sewa).", "metadata": {"title": "Agrarian Law"}}]}
        if "tax" in q_lower and "rate" in q_lower:
            return {"results": [{"text": "The standard Corporate Income Tax (PPh Badan) rate in Indonesia is 22%.", "metadata": {"title": "Harmonized Tax Law"}}]}
        if "cafe" in q_lower:
            return {"results": [{"text": "Restaurants and cafes are open to 100% foreign ownership. Using a local nominee is illegal and risky. PT PMA is the compliant path, requiring 10B IDR capital.", "metadata": {"title": "Investment Negative List"}}]}
...
async def run_evaluation():
    ...
    # Init RAG
        ...
        async def mock_llm_response(chat, message, system_prompt="", model_tier=0):
            ...
            if "Provide a final" in message or "Based on" in message:
                return "Based on the documents: The minimum paid-up capital for a PT PMA is 10 billion IDR. Investor KITAS holders cannot perform manual labor. Foreigners cannot own Freehold land.", "mock-model"
            
            # If message prompts for tool use (initial prompt), simulate tool call thought
            if "User Query:" in message:
                return "THOUGHT: I need to check the requirements.\nACTION: vector_search(query='capital pma')", "mock-model"
    ...
    for case in dataset:
        ...
        fact_score = 100
        if missing_facts: fact_score -= (len(missing_facts) / len(case['expected_facts']) * 100)
        if has_forbidden: fact_score = 0
        
        # 2. LLM Judge
        judge_result = {"score": 0, "reasoning": "No API Key"}
        if judge:
            judge_result = await judge.evaluate(case['question'], answer_text, case['expected_facts'])
