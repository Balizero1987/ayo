...
-- Check if migration already executed (log only, don't throw exception)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM migration_log WHERE migration_name = '008_v5_3_ultra_hybrid') THEN
        RAISE NOTICE 'Migration 008_v5_3_ultra_hybrid already executed, will update existing records';
    END IF;
END $$;
...
-- Create users table if not exists (required for this migration)
...
-- Create without FK first (FK added conditionally below based on users.id type)
CREATE TABLE IF NOT EXISTS knowledge_feedback (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID, -- FK added conditionally based on users.id type
...
-- Add FK for knowledge_feedback.user_id only if users.id is UUID type
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        WHERE constraint_name = 'knowledge_feedback_user_id_fkey'
        AND table_name = 'knowledge_feedback'
    ) THEN
        ALTER TABLE knowledge_feedback
        ADD CONSTRAINT knowledge_feedback_user_id_fkey
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;
        RAISE NOTICE 'Added FK constraint for knowledge_feedback.user_id';
    END IF;
END $$;
...
-- Query analytics for performance monitoring (FK added conditionally)
...
-- Add FK for query_analytics.user_id only if users.id is UUID type
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'query_analytics_user_id_fkey'
        AND table_name = 'query_analytics'
    ) THEN
        ALTER TABLE query_analytics
        ADD CONSTRAINT query_analytics_user_id_fkey
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL;
        RAISE NOTICE 'Added FK constraint for query_analytics.user_id';
    END IF;
END $$;
