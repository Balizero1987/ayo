...
class FrontendScribe:
    ...
    def scan_codebase(self) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        print(f"{Colors.OKCYAN}📚 Scanning Frontend Codebase...{Colors.ENDC}")

        # 1. Scan Pages (app directory - check both src/app and app)
        ...
        # 2. Scan Components (check both src/components and components)
        ...
        # 3. Scan API Calls (check both src/lib and lib)
        api_dir = self.frontend_dir / "src" / "lib"
        if not api_dir.exists():
            api_dir = self.frontend_dir / "lib"
        if api_dir.exists():
    ...
    def run(self):
        ...
        print("║   THE SCRIBE FRONTEND: DOCUMENTATION   ║")
        print(f"╚════════════════════════════════════════╝{Colors.ENDC}")

        pages, components, api_calls = self.scan_codebase()

        print(f"{Colors.OKGREEN}✔ Found {len(pages)} Pages{Colors.ENDC}")
        print(f"{Colors.OKGREEN}✔ Found {len(components)} Components{Colors.ENDC}")
        print(f"{Colors.OKGREEN}✔ Found {len(api_calls)} API Functions{Colors.ENDC}")
