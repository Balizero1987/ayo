#!/usr/bin/env python3
"""
Ingest Training Conversations into Qdrant (REST API version)
-------------------------------------------------------------
...
def build_context_injection(metadata: dict, lang: str = "id") -> str:
    ...
    # Format: [CONTEXT: TRAINING - CATEGORY X - TOPIC Y - LANG Z]
    parts = ["TRAINING", f"CATEGORY {category}", f"TOPIC {topic}"]
    if visa_type:
        parts.insert(2, f"VISA {visa_type}")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("TRAINING CONVERSATIONS INGESTION (REST API)")
    logger.info("Semantic Chunking + Context Injection")
    logger.info("=" * 60)
